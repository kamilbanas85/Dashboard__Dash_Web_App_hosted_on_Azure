...
dash_app.layout = html.Div(children = [
  ...
  html.Div([
    ...
    dcc.Store(id="store_Demand_02"),  # store that holds the data reference
...
@dash_app.callback(ServersideOutput("store_Demand", "data", session_check=False, backend=fss),
                   ServersideOutput("store_Demand_02", "data", session_check=False, backend=fss),
                   Trigger("trigger", "n_intervals"), memoize=True)
def update_data():
    
    Demand_Electricity_Texas =\
                Download_And_Process_Data(Authenication = 'password', returnData = True)
    Demand_Electricity_Texas_02 = Demand_Electricity_Texas.copy()*2   
    return Demand_Electricity_Texas, Demand_Electricity_Texas_02  # put your update logic here
...
def update_figure(freqency, data_02):
